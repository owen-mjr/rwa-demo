...
namespace RwaWebApi.Data
{
    public class ListConcertRepository : IConcertRepository
    {
        private List<Concert> _concertList;

        public Task<Concert?> GetConcertByIdAsync(int id)
        {
            return Task.FromResult(_concertList.Find(x => x.Id == id));
        }

        public Task<ICollection<Concert>> GetConcertsByIdAsync(ICollection<int> ids)
        {
            return Task.FromResult<ICollection<Concert>>(_concertList.Where(x => ids.Contains(x.Id)).ToList());
        }

        public Task<ICollection<Concert>> GetUpcomingConcertsAsync(int count)
        {
            return Task.FromResult<ICollection<Concert>>(_concertList.Where(c => c.StartTime > DateTimeOffset.UtcNow && c.IsVisible)
        ...
        {
            if(_concertList.Exists(x => x.Id == newConcert.Id))
            {
                //Concert is already added throw an error
                throw new InvalidOperationException($"A concert with the id {newConcert.Id} already exists");
            }
            else
            {
                _concertList.Add(newConcert);
            }
            return Task.FromResult(CreateResult.SuccessResult(newConcert.Id));
        }

        public Task<UpdateResult> UpdateConcertAsync(Concert model)
        {
            if (_concertList.Exists(x => x.Id == model.Id))
            {
                var index = _concertList.FindIndex(x => x.Id == model.Id);
                _concertList.RemoveAt(index);
                _concertList.Add(model);
            }
            else
            {
                //Concert does not exist throw an error
                throw new InvalidOperationException($"A concert with the id {model.Id} does not exists");
            }
        ...
        {
            if (_concertList.Exists(x => x.Id == id))
            {
                var index = _concertList.FindIndex(x => x.Id == id);
                _concertList.RemoveAt(index);
            }
            else
            {
                //Concert does not exist throw an error
                throw new InvalidOperationException($"A concert with the id {id} does not exists");
            }
            return Task.FromResult(DeleteResult.SuccessResult());
        }

        public void Initialize()
        {
            _concertList = new List<Concert>();
            ...
            for (var i = 0; i < 1000; i++)
            {
                ...
                _concertList.Add(new Concert {Id = i + 1, Artist = artist.Name, Genre = artist.Genre, Location = location, Price = price, Title = artist.Tour, Description = artist.Description, StartTime = startTime, IsVisible = true, CreatedBy = "System", CreatedOn = DateTime.UtcNow, UpdatedBy = "System", UpdatedOn = DateTime.UtcNow });
            }
        }
    }
}
